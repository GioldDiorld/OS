...
PUBLIC void schedule()
{
    ...
    while (!found) {
        for (i = 0; i < NR_SCHED_LEVELS; i++) {
            for (p = &FIRST_PROC; p <= &LAST_PROC; p++) {
                if (p->p_flags == 0 && p->ticks > 0 && p->ticks_in_slice < slice_time[i] && p->sched_level == i) {
                    p_proc_ready = p;
                    found = 1;
                    break;
                }
        ...
        if (!found) {
            for (p = &FIRST_PROC; p <= &LAST_PROC; p++) {
                if (p->p_flags == 0 && p->sched_level < NR_SCHED_LEVELS - 1) {
                    p->sched_level++;
                    p->ticks_in_slice = 0;
                }
                else if (p->p_flags == 0 && p->sched_level == NR_SCHED_LEVELS - 1) {
                    p->ticks_in_slice = 0;
                }
            }
            int all0 = 1;
            for (p = &FIRST_PROC; p <= &LAST_PROC; p++) {
                if (p->p_flags == 0 && p->ticks) {
                    all0 = 0;
                }
            }
            if (all0) {
