...
PUBLIC int do_fork()
{
	...
	*p = proc_table[pid];
    ...
    p->page_num = caller_T_size/PAGE_SIZE+(caller_T_size%PAGE_SIZE?1:0);
	int child_base = alloc_page(p->page_num);
    p->base_addr = child_base;
...
//	free_mem(pid);
   free_pages(p->base_addr, p->page_num);
